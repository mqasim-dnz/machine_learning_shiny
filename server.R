...
server <- function(input, output,session) {
  ...
  reactive ({ message(is.null(input$fileIn)) })
  
  rawdata <- reactive({
    inFile <- input$fileIn
    if (is.null(inFile)) { holder <- datasets[[input$dataset]] }
    else { holder <- read.csv(inFile$datapath) }
    return(holder)
  })
  ...
  makeReactiveBinding('dataTest') # Un-commented
  ...
  observe({
    ...
    if(lvls<10|(c!='numeric'&c!='integer')){
      ...
      modelType <<-'Regression'
      if(input$chk_logY==TRUE){df2$y <- log(df2$y+0.1)}
    }
